...
@bot.event
async def on_ready():
    print(f"{E['crystal']} Witchy Wisdom is online as {bot.user}")
...
async def shuffle(ctx):
    ...
    embed.set_footer(text=f"{E['spark']} Intention renewed • Witchy Wisdom Tarot Bot")
...
async def meaning(ctx, *, card_name: str):
    ...
    embed.add_field(name=f"Guidance {E['light']}", value=card.get("guidance", "—"), inline=False)
    embed.add_field(name=f"Call to Action {E['crystal']}", value=card.get("call_to_action", "—"), inline=False)
    embed.set_footer(text=f"{E['spark']} Witchy Wisdom Tarot Bot")
    await ctx.send(embed=embed)
...
async def wisdom(ctx):
    if not await channel_check(ctx): return
    embed = discord.Embed(
        title=f"{E['spark']} Witchy Wisdom Tarot Bot Commands",
        color=0x9370DB,
...
async def draw_many(ctx, number_of_cards: int):
    ...
    embed.set_footer(text=f"{E['spark']} Trust your intuition • Witchy Wisdom Tarot Bot")
    await ctx.send(embed=embed)
...
async def celtic_cross(ctx):
    ...
    embed.set_footer(text=f"{E['spark']} Trust the flow of insight • Witchy Wisdom Tarot Bot")
    await ctx.send(embed=embed)
