...
def draw_unique_cards(num_cards: int):
    """Draw a given number of unique tarot cards."""
...
def find_cards(query: str, max_fuzzy: int = 6):
    ...
    return [c for c in tarot_cards if normalize_name(c["name"]) in fuzzy_names]
...
async def read(ctx, *, question: str):
    ...
    cards = draw_unique_cards(len(positions))
    embed = discord.Embed(
